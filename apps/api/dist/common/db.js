...
for (const user of defaultUsers) {
    ...
    if (!existing) {
        db.prepare("INSERT INTO users (id, full_name, username, password_hash, role, created_at) VALUES (?, ?, ?, ?, ?, ?)").run(user.id, user.fullName, user.username, bcrypt.hashSync("admin1234", 10), user.role, now);
    }
}
...
for (const item of makerCheckerDefaults) {
    db.prepare("INSERT OR IGNORE INTO configurations (key, value) VALUES (?, ?)").run(item.key, item.value);
}
const shareholdersCount = db
    .prepare("SELECT COUNT(*) as count FROM shareholders")
    .get();
if (!shareholdersCount.count) {
    db.prepare("INSERT INTO shareholders (id, full_name_en, full_name_am, shares, is_high_power) VALUES (?, ?, ?, ?, ?)").run("sh-1001", "Abebe Kebede", "አበበ ከበደ", 250000, 1);
    db.prepare("INSERT INTO shareholders (id, full_name_en, full_name_am, shares, is_high_power) VALUES (?, ?, ?, ?, ?)").run("sh-1002", "Marta Solomon", "ማርታ ሰለሞን", 75000, 0);
}
const candidatesCount = db.prepare("SELECT COUNT(*) as count FROM candidates").get();
if (!candidatesCount.count) {
    db.prepare("INSERT INTO candidates (id, name, position) VALUES (?, ?, ?)").run("c-01", "Samuel Girma", "Board Chair");
}
export { db };
