...
import { z } from "zod";
import { requireAuth, requireRoles, signToken } from "../common/auth";
import { writeAuditLog } from "../common/audit";
import { db } from "../common/db";
import { emitDashboardRefresh } from "../common/realtime/socket";
import { buildDashboardSnapshot } from "./dashboard/service";
import { createShareholder, editShareholder, getShareholders, removeShareholder, shareholderSchema, } from "./shareholders/service";
function configEnabled(key) {
    const row = db.prepare("SELECT value FROM configurations WHERE key = ?").get(key);
...
function resolveInfluentialFlag(manualFlag, shares) {
...
async function createExcelBuffer(headers, rows, sheetName) {
...
function createPdfBuffer(title, headers, rows) {
...
export function registerRoutes(app) {
    ...
    app.get("/api/v1/dashboard/snapshot", requireAuth, (_req, res) => {
    ...
    app.post("/api/v1/shareholders", requireAuth, requireRoles(["SUPER_ADMIN"]), (req, res) => {
    ...
    app.put("/api/v1/shareholders/:id", requireAuth, requireRoles(["SUPER_ADMIN"]), (req, res) => {
        const parsed = shareholderSchema.safeParse(req.body);
        if (!parsed.success) {
            res.status(400).json({ message: "Invalid payload", issues: parsed.error.issues });
            return;
    ...
    app.delete("/api/v1/shareholders/:id", requireAuth, requireRoles(["SUPER_ADMIN"]), (req, res) => {
    ...
    app.patch("/api/v1/config", requireAuth, requireRoles(["SUPER_ADMIN"]), (req, res) => {
        const parsed = z
            .object({
    ...
    app.post("/api/v1/attendance/mark", requireAuth, requireRoles(["SUPER_ADMIN", "ATTENDANCE_MAKER"]), (req, res) => {
        ...
        const id = randomUUID();
        db.prepare("INSERT INTO attendance_records (id, shareholder_id, marked_by, approved_by, status, notes, timestamp) VALUES (?, ?, ?, ?, ?, ?, ?)").run(id, parsed.data.shareholderId, req.user?.id, makerChecker ? null : req.user?.id, status, parsed.data.notes ?? null, new Date().toISOString());
        writeAuditLog({
            userId: req.user?.id,
    ...
    app.get("/api/v1/attendance", requireAuth, (req, res) => {
        ...
        if (status) {
            const rows = db
                .prepare("SELECT id, shareholder_id, marked_by, approved_by, status, notes, timestamp FROM attendance_records WHERE status = ? ORDER BY timestamp DESC")
                .all(status);
            res.json(rows);
            return;
        }
        const rows = db
            .prepare("SELECT id, shareholder_id, marked_by, approved_by, status, notes, timestamp FROM attendance_records ORDER BY timestamp DESC")
            .all();
        res.json(rows);
    });
    app.post("/api/v1/attendance/approve", requireAuth, requireRoles(["SUPER_ADMIN", "ATTENDANCE_CHECKER"]), (req, res) => {
        ...
        db.prepare("UPDATE attendance_records SET status = ?, approved_by = ? WHERE id = ?").run(status, req.user?.id, parsed.data.attendanceId);
        writeAuditLog({
            userId: req.user?.id,
    ...
    app.post("/api/v1/attendance/reverse", requireAuth, requireRoles(["SUPER_ADMIN"]), (req, res) => {
        ...
        db.prepare("UPDATE attendance_records SET status = 'REJECTED', notes = ? WHERE id = ?").run(`Reversed: ${parsed.data.reason}`, parsed.data.attendanceId);
        writeAuditLog({
            userId: req.user?.id,
    ...
    app.post("/api/v1/votes/encode", requireAuth, requireRoles(["SUPER_ADMIN", "VOTE_ENCODER"]), (req, res) => {
        const parsed = z
        ...
        const status = makerChecker ? "PENDING" : "APPROVED";
        const id = randomUUID();
        db.prepare("INSERT INTO votes (id, shareholder_id, candidate_id, shares_used, encoded_by, approved_by, status, correction_history, timestamp) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)").run(id, parsed.data.shareholderId, parsed.data.candidateId, parsed.data.sharesUsed, req.user?.id, makerChecker ? null : req.user?.id, status, "[]", new Date().toISOString());
        writeAuditLog({
            userId: req.user?.id,
    ...
    app.get("/api/v1/votes", requireAuth, (req, res) => {
        ...
        if (status) {
            const rows = db
                .prepare("SELECT id, shareholder_id, candidate_id, shares_used, encoded_by, approved_by, status, timestamp FROM votes WHERE status = ? ORDER BY timestamp DESC")
                .all(status);
            res.json(rows);
            return;
        }
        const rows = db
            .prepare("SELECT id, shareholder_id, candidate_id, shares_used, encoded_by, approved_by, status, timestamp FROM votes ORDER BY timestamp DESC")
            .all();
        res.json(rows);
    });
    app.post("/api/v1/votes/approve", requireAuth, requireRoles(["SUPER_ADMIN", "VOTE_CHECKER"]), (req, res) => {
        ...
        const status = parsed.data.approve ? "APPROVED" : "REJECTED";
        db.prepare("UPDATE votes SET status = ?, approved_by = ? WHERE id = ?").run(status, req.user?.id, parsed.data.voteId);
        writeAuditLog({
            userId: req.user?.id,
    ...
    app.post("/api/v1/votes/reverse", requireAuth, requireRoles(["SUPER_ADMIN"]), (req, res) => {
        const parsed = z.object({ voteId: z.string(), reason: z.string().min(3) }).safeParse(req.body);
        if (!parsed.success) {
            res.status(400).json({ message: "Invalid payload" });
            return;
        }
        const previous = db.prepare("SELECT id, status, correction_history FROM votes WHERE id = ?").get(parsed.data.voteId);
        if (!previous) {
            res.status(404).json({ message: "Vote record not found" });
            return;
        }
        const history = JSON.parse(previous.correction_history);
        history.push(`${new Date().toISOString()} reversed: ${parsed.data.reason}`);
        db.prepare("UPDATE votes SET status = 'REJECTED', correction_history = ? WHERE id = ?").run(JSON.stringify(history), parsed.data.voteId);
        writeAuditLog({
            userId: req.user?.id,
    ...
    app.get("/api/v1/reports/attendance", requireAuth, async (req, res) => {
        ...
        const rows = db
            .prepare("SELECT id, shareholder_id, status, marked_by, approved_by, timestamp FROM attendance_records ORDER BY timestamp DESC")
            .all();
    ...
    app.get("/api/v1/reports/votes", requireAuth, async (req, res) => {
        const format = typeof req.query.format === "string" ? req.query.format : "csv";
        const rows = db
            .prepare("SELECT id, shareholder_id, candidate_id, shares_used, status, encoded_by, approved_by, timestamp FROM votes ORDER BY timestamp DESC")
            .all();
        const headers = [
            "Record ID",
            "Shareholder ID",
