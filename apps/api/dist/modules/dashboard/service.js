...
export function buildDashboardSnapshot() {
    ...
    const attendancePending = db
        ...
        .get();
    const totalVotes = db.prepare("SELECT COUNT(*) as count FROM votes WHERE status = 'APPROVED'").get();
    const votingPending = db.prepare("SELECT COUNT(*) as count FROM votes WHERE status = 'PENDING'").get();
    return {
        shareholders: {
